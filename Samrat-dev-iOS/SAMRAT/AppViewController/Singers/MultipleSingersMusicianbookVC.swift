...
class MultipleSingersMusicianbookVC: UIViewController, AlertViewDelegate {
...
//        self.btnBookNow.setTitle(Localized("bookNow").uppercased(), for: .normal)
        ...
        segmentControl.font = UIFont.systemFont(ofSize: 14, weight: .bold)
...
//                self.navigationController?.pushViewController(musicainVc, animated: true)
                ...
                if self.selectedMusician.count == 0 {
                    ...
                    onlinePaymentVc.isFromMultiple = true
                    onlinePaymentVc.geSelectedDate = self.selectedBookingDate
                    onlinePaymentVc.selectedSinger = nil
    ...
    func apiGetMusician() {
        ...
        let dateobj = engToArb(str:dateFormatter.string(from: (selectedBookingDate)!))
        ...
        APIManager.handler.PostRequest(url: ApiUrl.getMusicians, params: ["user_id": SamratGlobal.loggedInUser()?.user?.id ?? 0,
                                                                          "singer_id": selectedSingers.map({"\($0.id ?? 0)"}).joined(separator: ","),
                                                                          "date": dateobj,
...
extension MultipleSingersMusicianbookVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section == 0 {
            ...
            headerView.segmentControl.removeTarget(self, action: #selector(onSegmentValueChange(_:)), for: .valueChanged)
            headerView.segmentControl.addTarget(self, action: #selector(onSegmentValueChange(_:)), for: .valueChanged)
        ...
        if  self.musicanObj?.data?.count ?? 0 > 0 {
            ...
            if let musicanData = self.musicanObj?.data {
                
                headerView.lblTitle.text = musicanData[section].name
                headerView.lblChoose.text = Language.shared.isArabic ?  (musicanData[section].selection_label_ar ?? "") : (musicanData[section].selection_label ?? "")
            }
        }
...
//        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        setSelection(indexPath: indexPath)
    }
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        if isSelectedDefault || ((self.musicanObj?.data?.count ?? 0) - 1) == section {
                ...
                DispatchQueue.main.async {
                    if SamratGlobal.loggedInUser()?.user != nil {
                        if self.isSelectedDefault == true {
                            ...
                            self.selectedSingers.forEach { singer in
                                musiModel.append(contentsOf: singer.musicians ?? []);
                            }
...
//                            }
                            var ids: [Int] = []
                            singerModel.musicians = []
                            musiModel.forEach { (model) in
                                if !(ids.contains(model.id ?? 0)) {
                                    ids.append(model.id ?? 0)
                                    singerModel.musicians?.append(model)
                                }
                            }
//                            singerModel.musicians = musiModel
                            print(singerModel.musicians as Any)
...
//                            self.fadeTo(onlilePaymentVc)
                            ...
                            if getSelectedCountry()?.country == "Kuwait" || getSelectedCountry()?.country == "Kuwait ar"
                            {
                                if self.categoriesDetails?.id == 1
                                {
                                    ...
                                    selectBandVc.selectedSinger = nil
                                    selectBandVc.geSelectedDate = self.selectedBookingDate
                                    ...
                                    selectBandVc.selectedSingers = self.selectedSingers
                                    selectBandVc.isFromMultiple = true
                                ...
                                else
                                {
                                    ...
                                    if singerModel.musicians?.count == 0
                                    {
                                    ...
                                    else
                                    {
                                        ...
                                        onlinePaymentVc.getSelectedMusician = self.selectedMusician
                                        onlinePaymentVc.isFromMultiple = true
                                        onlinePaymentVc.isFromCustom = false
                            ...
                            } else if self.selectedMusician.count == 0 {
                                AlertView.instance.showAlert(title: Localized("alert"), message: NSAttributedString(string: Localized("pSelectMusician")), alertType: .oneButton)
                                AlertView.instance.alertViewDelegate = self
                            } else {
//                                let onlinePaymentVc = OnlinePaymentVC()
...
extension MultipleSingersMusicianbookVC: UIScrollViewDelegate {

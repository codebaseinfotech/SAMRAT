...
open class DropDown : UITextField{
    ...
    public func showList() {
        ...
        //table.layer.borderWidth = UIEdgeInsets(top: 0, left: 2, bottom: 2, right: 2)
        table.layer.borderColor = arrowColor.cgColor
        ...
        var y = self.pointToParent.y+self.frame.height - 7
        
       
        if height < (keyboardHeight+tableheightX){
            y = self.pointToParent.y - tableheightX + 7
            
            self.addLeftBorder(with: arrowColor, andWidth: 2)
            self.addRightBorder(with: arrowColor, andWidth: 2)
            self.addTopBorder(with: arrowColor, andWidth: 2)
            ...
            self.addLeftBorder(with: arrowColor, andWidth: 2)
            self.addRightBorder(with: arrowColor, andWidth: 2)
            self.addBottomBorder(with: arrowColor, andWidth: 2)
            if #available(iOS 11.0, *) {
